"""
模型主架構：Pretrained DenseNet201 + Transformer
...
class cfg(object):
    ...
    pretrained_model = 'weights/densenet_synthetic_1000k.pt'  # 已事先用「合成序號」預訓練過的 backbone 權重
...
class CSCIdentifier(pl.LightningModule):
    ...
    def __init__(self):
        ...
        # 修改 densenet201 部份結構，使當輸入圖像寬高為 480x80 時，輸出 shape=(batch, channel=1920, height=2, width=58)
        self.backbone.features.transition2.pool.stride = (2, 1)
        self.backbone.features.transition3.pool.stride = (2, 1)

        self.backbone.load_state_dict(torch.load(cfg.pretrained_model), strict=True)

        self.map_to_seq = torch.nn.Linear(1920 * 2, cfg.d_model)
    ...
    def forward(self, images, targets):
        conv = self.backbone.features(images)  # (batch, channel=1920, height=2, width=58)
