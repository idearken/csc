"""
模型主架構：Pretrained Inception_v3 + Transformer
...
from backbone_inception import inception_v3
...
class cfg(object):
    ...
    pretrained_model = 'weights/inception_synthetic_1000k.pt'  # 已事先用「合成序號」預訓練過的 backbone 權重
...
class CSCIdentifier(pl.LightningModule):
    ...
    def __init__(self):
        super().__init__()
        self.backbone = inception_v3(pretrained=False, aux_logits=False, init_weights=True)
