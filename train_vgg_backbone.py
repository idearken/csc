"""
用大量合成資料 (synthetic_training_1000k) 預訓練 VGG 模型。
...
from backbone_vgg import vgg16_bn
...
class SynClassifier(pl.LightningModule):
    ...
    def __init__(self):
        ...
        self.classifier = nn.Linear(512 * 2, cfg.num_class)
        self.criterion = nn.BCEWithLogitsLoss()

    def forward(self, images):
        x = self.backbone(images)  # (batch, channel=512, height=2, width=58)
...
if __name__ == '__main__':
    ...
    # eval_test_dataset('lightning_logs/version_2/checkpoints/epoch=4 loss=0.0093 val_loss=0.0005 val_acc=0.9977.ckpt',
    #                   f'../csc-datasets/analysis/{pth.splitext(pth.basename(__file__))[0]}')

    # get_backbone_from_ckpt('lightning_logs/version_2/checkpoints/epoch=4 loss=0.0093 val_loss=0.0005 val_acc=0.9977.ckpt',
    #                        'weights/vgg_synthetic_1000k.pt')
